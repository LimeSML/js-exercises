...
test.describe("simple todo app", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("http://localhost:3000/ch15.11-15/ex05");
  });
  ...
  test("add multiple todos", async ({ page }) => {
    ...
    await expect.poll(async () => await countToDos(page), { timeout: 1000 }).toBe(2);
  ...
  test("delete todo", async ({ page }) => {
    ...
    await expect.poll(async () => await countToDos(page), { timeout: 1000 }).toBe(1);
  ...
  test("persist todos in localStorage", async ({ page }) => {
    await addToDo(page, "質問表に質問を記載する");
    await addToDo(page, "練習問題を完了する");
    await checkToDo(page, 1);
  ...
  test("synchronize todos across tabs", async ({ browser }) => {
    // 同じ browserContext 内で作成された page は、localStorage などを共有する
    const context = await browser.newContext();
    const page1 = await context.newPage();
    const page2 = await context.newPage();

    await page1.goto("http://localhost:3000/ch15.11-15/ex04");
    await page2.goto("http://localhost:3000/ch15.11-15/ex04");

    await addToDo(page1, "質問表に質問を記載する");
    await addToDo(page1, "練習問題を完了する");
    await checkToDo(page1, 1);

    expect(await countToDos(page1)).toBe(2);

    await expect.poll(async () => await countToDos(page2), { timeout: 10000 }).toBe(2);
