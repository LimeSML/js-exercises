...
describe('setAlarm', () => {
    ...
    it('should do nothing in other states', () => {
        ...
        expect(setAlarm(State.ALARM_SOUNDING)).toEqual([Action.NONE, State.ALARM_SOUNDING]);
...
describe('cancelAlarm', () => {
    ...
    it('should stop alarm when in ALARM_SOUNDING state', () => {
        expect(cancelAlarm(State.ALARM_SOUNDING)).toEqual([Action.STOP_ALARM, State.NORMAL]);
    });
...
describe('reachedToAlarmTime', () => {
    it('should sound alarm when in ALARM_SET state', () => {
        expect(reachedToAlarmTime(State.ALARM_SET)).toEqual([Action.SOUND_ALARM, State.ALARM_SOUNDING]);
    });
    it('should do nothing in other states', () => {
        expect(reachedToAlarmTime(State.NORMAL)).toEqual([Action.NONE, State.NORMAL]);
        expect(reachedToAlarmTime(State.ALARM_SOUNDING)).toEqual([Action.NONE, State.ALARM_SOUNDING]);
        expect(reachedToAlarmTime(State.SNOOZING)).toEqual([Action.NONE, State.SNOOZING]);
    });
});

describe('snooze', () => {
    it('should snooze when in ALARM_SOUNDING state', () => {
        expect(snooze(State.ALARM_SOUNDING)).toEqual([Action.STOP_ALARM, State.SNOOZING]);
    });
    it('should do nothing in other states', () => {
...
describe('elapseSnoozeTime', () => {
    it('should sound alarm again when snooze time elapses', () => {
        expect(elapseSnoozeTime(State.SNOOZING)).toEqual([Action.SOUND_ALARM, State.ALARM_SOUNDING]);
    });
    it('should do nothing in other states', () => {
        expect(elapseSnoozeTime(State.NORMAL)).toEqual([Action.NONE, State.NORMAL]);
        expect(elapseSnoozeTime(State.ALARM_SET)).toEqual([Action.NONE, State.ALARM_SET]);
        expect(elapseSnoozeTime(State.ALARM_SOUNDING)).toEqual([Action.NONE, State.ALARM_SOUNDING]);
    });
});