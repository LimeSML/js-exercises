...
describe('pow', () => {
    ...
    it('should throw an error for negative exponents', () => {
        ...
        expect(() => pow(5, -10)).toThrow('exponent must be a non-negative integer');
    });

    it('should throw error for non-integer exponents', () => {
        expect(() => pow(2, 1.5)).toThrow('exponent must be a non-negative integer');
        expect(() => pow(3, NaN)).toThrow('exponent must be a non-negative integer');
    });
});