...
describe('littleToBigEndian', () => {
    ...
    it('should throw an Error if input is not Uint32Array', () => {
        expect(() => littleToBigEndian([0x11223344])).toThrow('input must be a Uint32Array');
        expect(() => littleToBigEndian(null)).toThrow('input must be a Uint32Array');
        expect(() => littleToBigEndian(undefined)).toThrow('input must be a Uint32Array');
        expect(() => littleToBigEndian(new Uint8Array([1,2,3,4]))).toThrow('input must be a Uint32Array');
    });
});
...
describe('bigToLittleEndian', () => {
    ...
    it('should throw an Error if input is not Uint32Array', () => {
        expect(() => bigToLittleEndian([0x44332211])).toThrow('input must be a Uint32Array');
    });
});