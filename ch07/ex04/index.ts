...
const totalChemistryInClassA = studentsInClassA.reduce((total, student) => total + student.chemistry, 0);
const averageChemistryInClassA = totalChemistryInClassA / studentsInClassA.length;
...
// 全体のgeographyの標準偏差
const averageGeography = data.reduce((sum, student) => sum + student.geography, 0) / data.length;
const varianceGeography = data.reduce((sum, student) => sum + Math.pow(student.geography - averageGeography, 2), 0) / data.length;
