...
function padding(message) {
    ...
    if (messageLength % 64 < 56) {
        paddedMessage = paddedMessage.concat(paddingBlock.slice(0, 56 - messageLength % 64));
    } else {
        paddedMessage = paddedMessage.concat(paddingBlock.slice(0, 64 + 56 - messageLength % 64));
    }
...
function compute(paddedMessage) {
    if (!Array.isArray(paddedMessage) || paddedMessage.length % 64 !== 0) {
        throw new Error('paddedMessage must be an array with length multiple of 64');
    }
    ...
    for (let i = 1; i <= N; i++) {
        for (let t = 0; t < 64; t++) {
            if (t < 16) {
                ...
                W[t] = (paddedMessage[p] << 24) + (paddedMessage[p + 1] << 16) + (paddedMessage[p + 2] << 8) + paddedMessage[p + 3];
            } else {
