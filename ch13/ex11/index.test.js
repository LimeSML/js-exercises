...
describe("retryWithExponentialBackoff", () => {
  ...
  it("should throw TypeError if func is not a function", async () => {
    await expect(retryWithExponentialBackoff(null, 3)).rejects.toThrow(
      TypeError,
    );
    await expect(retryWithExponentialBackoff(123, 3)).rejects.toThrow(
      "func must be a function",
    );
  });

  it("should throw RangeError if maxRetry is negative or not finite", async () => {
    await expect(retryWithExponentialBackoff(() => {}, -1)).rejects.toThrow(
      RangeError,
    );
    await expect(
      retryWithExponentialBackoff(() => {}, Infinity),
    ).rejects.toThrow("maxRetry must be a non-negative number");
    await expect(retryWithExponentialBackoff(() => {}, NaN)).rejects.toThrow(
      RangeError,
    );
  });
  ...
  it("should resolve if func succeeds on first try", async () => {
    ...
    await expect(retryWithExponentialBackoff(mockFn, 3)).resolves.toBe(
      "success",
    );
