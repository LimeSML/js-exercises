...
describe("countWeekdays", () => {
  ...
  test("should throw an error for invalid date format", () => {
    expect(() => countWeekdays("2025-8-01", "2025-08-10")).toThrow(
      "invalid date format",
    );
    expect(() => countWeekdays("2025-08-01", "2025/08/10")).toThrow(
      "invalid date format",
    );
  });
});
...
describe("getWeekdayByLocale", () => {
  ...
  test("should throw an error for invalid date format", () => {
    expect(() => getWeekdayByLocale("2025/08/06", "en-US")).toThrow(
      "invalid date format",
    );
    expect(() => getWeekdayByLocale("2025-8-06", "en-US")).toThrow(
      "invalid date format",
    );
  });
});
